...
static int validate_user_data(User user, int check_id_existence) {
    ...
    if (strlen(user.username) < MIN_USERNAME_LEN || strlen(user.username) > MAX_USERNAME_LEN) {
        LOG_WARNING("El nombre de usuario debe tener entre %d y %d caracteres", MIN_USERNAME_LEN, MAX_USERNAME_LEN);
        return -1;
    }

    if (strlen(user.password) < MIN_PASSWORD_LEN || strlen(user.password) > MAX_PASSWORD_LEN) {
        LOG_WARNING("La contraseña debe tener entre %d y %d caracteres", MIN_PASSWORD_LEN, MAX_PASSWORD_LEN);
        return -1;
    }
...
int create_user_service(User user) {
    if (validate_user_data(user, 1) != 0) return -1;
...
int update_user_service(User user) {
    ...
    if (validate_user_data(user, 0) != 0) return -1;
