...
char* pos_get_barcode_input_ui() {
    ...
    printf("Ingrese el código de barras del producto (13 caracteres o '0' para terminar): ");
    scanf("%13s", barcode);
...
int get_quantity_input_ui(int isAdd) {
    ...
    if (quantity > 100) {
        show_error_ui("Cantidad demasiado alta. Por favor, ingrese un valor menor o igual a 100.");
        return -1; // Indicar error
    }
...
int show_confirm_product(Product* product) {
    ...
    printf("%s\nprecio: %.2f\nPromoción: %s\n", product->name, product->price, promo_type_to_string(product->promo_type));
    ...
    while (true) {
        ...
        if (confirm == 's' || confirm == 'S') {
            ...
            printf("Entrada inválida. Por favor, ingrese 's' para sí o 'n' para no: ");
        }
    }
...
void show_cart_invoice_ui(float subtotal, float tax, float savings, float total) {
...
void generate_ticket_ui(Cart* cart, Breakdown breakdown, float payment, float change, char* nameBusiness) {
